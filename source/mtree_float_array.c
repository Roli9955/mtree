...
Datum mtree_float_array_union(PG_FUNCTION_ARGS) {
	...
	// 		} else {
	// 			float exact_distance = mtree_float_array_exact_distance(entries[i], entries[j]);
	...
	out->coveringRadius += mtree_float_array_distance_internal(entries[0], entries[1]);
...
Datum mtree_float_array_picksplit(PG_FUNCTION_ARGS) {
	...
	case SamplingMinCoveringSum:
		for (int i = 0; i < trialCount; ++i) {
			...
			for (int currentIndex = 0; currentIndex < maxOffset; currentIndex++) {
				float leftDistance = get_float_array_distance(maxOffset, entries, distances, leftCandidateIndex, currentIndex);
				float rightDistance = get_float_array_distance(maxOffset, entries, distances, rightCandidateIndex, currentIndex);
	...
	case SamplingMinOverlapArea:
		for (int i = 0; i < trialCount; i++) {
			leftCandidateIndex = ((int)random()) % (maxOffset - 1);
			rightCandidateIndex = (leftCandidateIndex + 1) + (((int)random()) % (maxOffset - leftCandidateIndex - 1));
			float distance = get_float_array_distance(maxOffset, entries, distances, leftCandidateIndex, rightCandidateIndex);
			float leftRadius = 0.0, rightRadius = 0.0;
	...
	case GuttmanPolyTime:
		for (int i = 0; i < maxOffset; i++) {
			for (int j = i + 1; j < maxOffset; j++) {
				...
				double currentOverlapArea = overlap_area_float(leftRadius, rightRadius, distance);
				if ((minOverlapArea == -1.0 || currentOverlapArea < minOverlapArea) && (currentOverlapArea != 0.0)) {
					minOverlapArea = (float)currentOverlapArea;
					elog(INFO, "I: %d, J: %d, LEFTRADIUS: %f, RIGHTRADIUS: %f, DISTANCE: %f", i, j, leftRadius, rightRadius, distance);
	...
	elog(INFO, "BELSO CSUCS B: %f, %f, %f, %f", unionLeft->data[0], unionLeft->data[1], unionLeft->data[2], unionLeft->coveringRadius);
	elog(INFO, "BELSO CSUCS J: %f, %f, %f, %f", unionRight->data[0], unionRight->data[1], unionRight->data[2], unionRight->coveringRadius);
